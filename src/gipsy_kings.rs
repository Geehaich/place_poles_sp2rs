...
#[derive(Clone, Debug)]
pub struct FullQr<T: ComplexField, R: DimMin<C>, C: Dim>
where
...
impl<T: ComplexField, R: DimMin<C>, C: Dim> FullQr<T, R, C>
where
DefaultAllocator: Allocator<T, R, C> + Allocator<T, R> + Allocator<T, DimMinimum<R, C>>,
{
    ...
    {
        let (nrows, _) = self.qr.shape_generic();
...
impl<T: ComplexField, D: DimMin<D, Output = D>> FullQr<T, D, D>
where
